...
#
#install the tests on all devices
print("\nInstalling all AndroidJUnitRunner tests on all connected devices..\n");
chdir "..";
$output = `gradle.bat installDebugAndroidTest`;
print "\n\toutput:   $output\n";
chdir "P2P_ON_OFF";
#
...
#	sleep 2;
	...
	$P2P_Device1 = `adb -s $device1 shell getprop ro.product.model`;
	$P2P_Device2 = `adb -s $device2 shell getprop ro.product.model`;
	print "\n\tsendInvite( $device2, $device1 )\n";
	sleep($delay);
	devices::sendInvite( $device2, $device1 );
